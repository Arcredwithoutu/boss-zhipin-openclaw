...
=== 注意注意注意注意注意 ===
* 另一个分支（代理版分支）的代码进度更靠前，优先使用代理版分支
* 老板平台监控很严格，在Base国外的服务器上使用需要架设代理：本地ssh到云主机上，走本地ip请求
...
在浏览器中登录 [Boss直聘](https://www.zhipin.com)，然后获取 Cookie：

1. 打开浏览器开发者工具（F12）
2. 切换到 Network 面板
3. 在 Boss直聘页面进行任意操作（如搜索岗位）
4. 点击任意一个请求，找到 Request Headers 中的 `Cookie` 字段
5. 复制完整值
